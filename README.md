...
### 与 WMS-AI 联动
- WMS-AI 与本系统共用 MongoDB 的 `permission_manager` 库，用本系统维护的角色/权限数据进行鉴权。
- WMS-AI 的页面访问与菜单显示主要依赖 `wms:xxx:yyy:view`（页面权限）；分配权限时建议包含对应模块下的 `*:view` 节点，否则 WMS-AI 可能提示无权限或隐藏入口。
- 分配权限后，如 WMS-AI 仍显示无权限，请重新登录 WMS-AI（或清理本地缓存）以刷新权限码列表。
